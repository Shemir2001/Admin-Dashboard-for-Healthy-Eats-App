...
try {
      ...
      await addDoc(collection(db, "Recipes"), { ...newRecipe, id: docRef.id });
...
<div className="w-full h-screen mx-auto p-2 shadow-lg overflow-hidden rounded-lg">
  <div className="flex justify-end  gap-4">
    ...
    <div className="flex space-x-4 justify-between  mb-6">
      <div>
      <button className="px-6 py-2 text-white rounded-lg cursor-pointer" style={{ backgroundColor: Colors.buttonBg }}>Save</button></div>
    <div>  <button onClick={handlePublish} className="px-6 py-2 text-white rounded-lg cursor-pointer" style={{ backgroundColor: Colors.buttonBg }}>
        {loading? <Spin /> : "Publish"}
      </button></div>
    </div>
  </div>
  <div className="flex gap-8 h-full">
    <div className="w-1/3 h-[80vh] overflow-y-auto p-4 flex flex-col space-y-16"style={{ backgroundColor: Colors.primaryBg }}>
      <div>
        <label className="block font-medium mb-2">Recipe Name</label>
        <input type="text" value={recipeName} onChange={(e) => setRecipeName(e.target.value)} className="w-full border rounded-lg p-2" />
      </div>
      <div>
      ...
      <div className="mb-4">
        ...
        {imageUrl && <img src={imageUrl} alt="Uploaded Preview" className="mt-2 h-40 rounded-lg shadow" />}
      </div>
    </div>
        ...
        {tempList.map((item, index) => (
          <li key={index} className="flex justify-between items-center bg-gray-100 px-3 py-2 rounded-lg mb-2">
            ...
            <div className="flex gap-2">
              ...
              <Button style={{ color: 'black' }} type="link" danger onClick={() => handleRemoveItem1(index, modalType)}>
